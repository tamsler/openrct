...
// $Id: com_class_chat_msg_input.php,v 1.3 2002/11/04 21:54:58 thomas Exp $
...
function gen_form($file1, $file2, $file3, $file4, $file5, $assembly_type, $offset) {
	...
	if(!$_POST['obj_id']) {
		...
		disp_simple_msg("List Class Team Chat Messages:");

		display_class_team_chat_messages($file1, $file3, $offset);
		...
		disp_simple_msg("List Class Group Chat Messages:");

		display_class_group_chat_messages($file2, $file3, $offset);
...
function display_class_group_chat_messages($file1, $file2, $offset) {
    ...
    $query = "select
              ...
              from rct_chat_log_groups where class_id='" . $_POST['obj_id'] . "' 
    ...
    if(0 == $numrows) {
        ...
        disp_msg("There are not group chat messages for this class");
	$db->close();
        unset($db);
        return;
    }
...
function display_class_team_chat_messages($file1, $file2, $offset) {
    ...
    $query = "select
              ...
              rct_date as \"Date\",
              rct_version as \"Version\"
              from rct_chat_log_teams where class_id='" . $_POST['obj_id'] . "'
              order by rct_date asc 
              limit " . RCT_LIMIT . " offset " . $offset;
    ...
    if(0 == $numrows) {
        ...
        disp_msg("There are not team chat messages for this class");
	$db->close();
        unset($db);
        return;
    }
...
function gen_chat_msg_delete($row, $file1, $file2) {
    ...
    printf("<td>
	    ...
	    <input type=hidden name=chat_id value='" . $row[0] . "'>
            <input type=hidden name=obj_id value='" . $row[3] . "'>
	    <input type=hidden name=access value='" . $_POST['access'] . "'>
	    <input type=hidden name=access_id value='" . $_POST['access_id'] . "'>
