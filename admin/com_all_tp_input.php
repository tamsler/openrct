...
// $Id: com_all_tp_input.php,v 1.5 2002/11/18 22:59:45 thomas Exp $
...
function gen_form($file1, $file2, $assembly_type, $person_access, $offset) {
	...
	gen_html_header("All Textpads:");

	if(TEAM == $assembly_type || TEAM_AND_GROUP == $assembly_type) {

	    disp_simple_msg("List Of All Team Textpads:");

	    display_all_textpads($file1, TEAM, $person_access, $offset);
	...
	if(GROUP == $assembly_type || TEAM_AND_GROUP == $assembly_type) {
	    ...
	    disp_simple_msg("List Of All Group Textpads:");

	    display_all_textpads($file1, GROUP, $person_access, $offset);
...
function display_all_textpads($file1, $assembly_type, $person_access, $offset) {
	...
	$query = "select * from rct_team_textpads_view
                  order by \"Date\" asc
	...
	$query = "select TTV.* from rct_team_textpads_view TTV, rct_classes C
                  where C.class_name=TTV.\"Class Name\" and
                  C.manager='" . $_POST['access_id'] . "' order by TTV.\"Date\" asc
                  limit " . RCT_LIMIT . " offset " . $offset;
    }
    elseif(GROUP == $assembly_type &&
	   ADMIN_ACCESS == $person_access) {

	$query = "select * from rct_group_textpads_view
                  order by \"Date\" asc
                  limit " . RCT_LIMIT . " offset " . $offset;
    }
    elseif(GROUP == $assembly_type &&
	...
	$query = "select GTV.* from rct_group_textpads_view GTV, rct_classes C
                  where C.class_name=GTV.\"Class Name\" and
                  C.manager='" . $_POST['access_id'] . "' order by GTV.\"Date\" asc
                  limit " . RCT_LIMIT . " offset " . $offset;
    }
	...
	disp_err_msg("ERROR: Couldn't get textpads from DB!");
	$db->close();
	unset($db);
	return;
    }
    ...
    if(0 == $numrows) {
        ...
        disp_msg("There are no textpads.");
	$db->close();
        unset($db);
        return;
    }
	...
	if($i % 2) {
	    ...
	    gen_tp_delete($myrow[0], $file1, $person_access, $assembly_type);
...
function gen_tp_delete($tp_id, $file, $person_access, $assembly_type) {

    printf("<td>");

    if(ADMIN_ACCESS == $person_access) {
	printf("<form method='post' action='delete_tp_verify.php'>");
    }
    elseif(MANAGER_ACCESS == $person_access) {
	printf("<form method='post' action='m_delete_tp_verify.php'>");
    }
...
function gen_next_previous($file1, $file2, $assembly_type, $person_access, $offset) {
    ...
    $count_query = "";
    if(TEAM == $assembly_type || TEAM_AND_GROUP == $assembly_type) {
	$count_query = "select count(\"ID\") from rct_team_textpads_view";
    }
    else {
	$count_query = "select count(\"ID\") from rct_group_textpads_view";
    }
